...
func TestConnectionInstanceStateUpgradeV0(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:     "Empty",
			state:    map[string]interface{}{"strategy_version": ""},
			expected: map[string]interface{}{"strategy_version": 0},
		},
		{
			name:     "Zero",
			state:    map[string]interface{}{"strategy_version": "0"},
			expected: map[string]interface{}{"strategy_version": 0},
		},
		{
			name:     "NonZero",
			state:    map[string]interface{}{"strategy_version": "123"},
			expected: map[string]interface{}{"strategy_version": 123},
		},
		{
			name:     "Invalid",
			state:    map[string]interface{}{"strategy_version": "foo"},
			expected: map[string]interface{}{"strategy_version": 0},
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			actual, err := connectionSchemaUpgradeV0(tt.state, nil)
			if err != nil {
